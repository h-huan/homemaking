package com.hm.module.infra.service.codegen.inner;
...
import com.hm.framework.excel.core.annotations.DictFormat;
import com.hm.framework.excel.core.convert.DictConvert;
import com.hm.framework.excel.core.util.ExcelUtils;
import com.hm.framework.mybatis.core.dataobject.BaseDO;
import com.hm.framework.mybatis.core.mapper.BaseMapperX;
import com.hm.framework.mybatis.core.query.LambdaQueryWrapperX;
import com.hm.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.hm.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.hm.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.hm.module.infra.enums.codegen.CodegenSceneEnum;
import com.hm.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.hm.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.hm.module.infra.framework.codegen.config.CodegenProperties;
...
import static com.hm.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.hm.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 hm-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 hm-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.hm.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/hm-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/hm-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "hm-module-${table.moduleName}/" + // 顶级模块
                "hm-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "hm-module-${table.moduleName}/" + // 顶级模块
                "hm-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "hm-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "hm-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "hm-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "hm-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
