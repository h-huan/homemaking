...
`HM_EVIDENCE_ROOT` 在后端启动时读取，生产模板为 `/var/lib/hm/evidence`，开发默认 `./data/homemaking-evidence`。生产必须使用发布目录之外的绝对路径，提前创建并赋予后端账号读写权限；Nginx 不得将它设置为 root/alias。数据库只保存元数据，备份和恢复时必须同时处理数据库与此目录。履约照片与评价图片均只支持 JPEG/PNG，单张最大 5 MB；履约照片每单最多 20 张，评价图片每条最多 6 张。服务端校验尺寸并重新编码去除 EXIF，查看接口核对客户、人员或有数据权限的管理员身份。
...
| 排班与产能 | 家政运营 → 排班 | 人员所属门店、可服务项目、行政区、日期班次，以及请假/休息审批；全区域需明确选择 *，未配置人员不会被自动派单 |
| 人员权限 | 家政运营 → 人员权限 | 给已有账号分配七类模板及门店范围；V004 升级后旧通用管理权限不会自动转换，需逐人分配 |
| 人员工作台 | 人员权限授予「服务人员」模板，再在排班页绑定账号 | 同租户有效后台用户；权限为 `homemaking:worker:read / fulfill / leave / income`，支持本人排班、可服务时间、请假休息申请、任务、收入与结算历史；客户微信账号不能作为人员后台账号使用 |
| 客户完工确认 | 品牌与渠道 → 客户完工确认期限 | 1–168 小时，默认 48；租户级业务配置，不是环境变量。服务人员提交后由客户确认或系统到期确认，进行中售后暂停自动确认 |
| 售后处理 | 家政运营 → 售后；小程序订单 → 申请售后 | V010 支持六类诉求、处理时间线和服务补救预约；沿用排班、文件、支付和通知配置，不新增环境变量。已有库按当前版本顺序执行 V010，全新库仍只导入 hm-init.sql |
| 评价管理 | 家政运营 → 评价；小程序订单 → 评价 | V011 支持双评分、标签、最多 6 张私有图片、回复、展示与推荐；沿用 `HM_EVIDENCE_ROOT`，不新增环境变量。已有库执行 V011，全新库仍只导入 hm-init.sql |
| 加盟商与合同 | 家政运营 → 加盟与合同，仅平台管理员 | V012 保存独立加盟档案、区域、合同与保证金流水。负责人手机号和结算账号复用 `HM_DATA_ENCRYPTION_KEY` 加密；密钥更换会导致既有密文无法解密。已有库执行 V012，全新库仍只导入 hm-init.sql |
