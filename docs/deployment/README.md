...
正式切换前须在 MySQL 8.0.16+ 演练迁移与恢复，并用自有微信、支付、短信账号及小程序真机验收；本地 H2 检查不能替代原生数据库验证。旧人员技能/区域/排班需按归档确认并录入，历史支付需人工对账。未启用模块须单独检查后再开放。
...
| 私有履约与评价图片 | `/var/lib/hm/evidence/`，由 `HM_EVIDENCE_ROOT` 指定，后端账号可读写，禁止 Nginx alias/root 公开 |
...
在**新建的空目标库**中，**只导入一次 [`sql/mysql/hm-init.sql`](../../sql/mysql/hm-init.sql)**。这份独立 SQL 已包含平台基础表、初始平台管理员、普通直营租户、家政菜单及当前全部结构；无需额外导入其他 SQL。
...
**已有数据库不能重新导入 hm-init.sql。** 仅依照[增量升级说明](../../sql/mysql/upgrades/README.md)执行尚未应用的版本，执行前停止写入并备份。部分 DDL 失败不能依靠事务整体回滚。旧家政数据迁移另见[迁移操作说明](../migration/runbook.md#旧库迁移)。
...
- 排班页先设置人员技能、行政区和日期班次；未配置时客户看到无可约时段，这是产能保护。人员账号需绑定到同租户后台用户，并在人员权限页授予「服务人员」模板。服务人员从工作台提交请假/休息，店长在排班页审批；待审核不影响产能，批准后才生效。工作台收入只读取真实分佣和结算流水。
- 工作台上传服务前后照片，客户仅能查看自己订单的凭证；确认私有照片落在 `HM_EVIDENCE_ROOT`，不在网页包或公共对象桶。服务人员提交完工后检查小程序出现客户确认和异常售后入口；客户确认或到期确认后订单才完成并进入结算。
- 分别创建补做、换人、重新上门、部分退款、全额退款和其他补偿售后。确认服务补救重新占用有效排班并回到人员工作台；客户可查看时间线和撤销待处理申请，处理人员不能越过门店数据范围。核对线上/线下退款都写入原收支与结算冲正链路。
- 用小程序完成一条含双评分、标签和图片的评价；上传中断时评价不得公开，重试完成后才能发布。客服可回复但不能审核展示，店长可按门店范围公开或推荐，财务不得查看评价。确认图片落在 `HM_EVIDENCE_ROOT`，小程序订单详情显示商家回复，推荐评价优先出现在小程序首页和官网。
...
评价收口没有新增秘密环境变量；私有图片目录 `HM_EVIDENCE_ROOT` 仍需保留并与数据库一起备份。数据库、Redis、微信和支付凭据继续使用原有私有配置。官网、套餐、分佣、排班在后台/数据库维护，不应写成前端秘密。结算“登记打款”只记录已经发生的人工打款，不会调用银行或微信转账接口。
