...
Admin operations require both system permissions and a bounded homemaking role template. Store-scoped roles filter detail access, writes, lists, totals and financial reports; workers remain restricted to their own tasks. Platform authority is an explicit global system_platform_operator grant, independent of tenant ID, operating mode and tenant roles. The original login tenant identifies the account partition; visit-tenant-id only selects a target after a live platform authorization check. The headquarters tenant uses an ordinary business package. Tenant user administration cannot mutate an active platform account. Grants, revocations and visits are audited; revocation takes effect on existing sessions.
