...
public class BrandingService {
    ...
    private Map<String,Object> profile(long tenant){var rows=repo.jdbc().queryForList("SELECT tenant_id,brand_name,website,logo,favicon,primary_color,login_title,login_background,home_modules,mini_app_id,mp_app_id,operation_mode,version FROM hm_tenant_profile WHERE tenant_id=?",tenant);
        return rows.isEmpty()?Map.of("tenant_id",tenant,"brand_name","HM 家政","logo","/hm-logo.svg","favicon","/hm-logo.svg","primary_color","#136f63","login_title","欢迎使用 HM 家政","home_modules","[\"services\"]","version",0):rows.get(0);}
    ...
    public void save(Brand b){check(Set.of("services","stores","workers","reviews","contact","banners").containsAll(b.homeModules()),"首页模块无效");
        ...
        int changed=repo.jdbc().update("UPDATE hm_tenant_profile SET brand_name=?,website=?,logo=?,favicon=?,primary_color=?,login_title=?,login_background=?,home_modules=?,mini_app_id=?,mp_app_id=?,operation_mode=?,pay_app_key=?,version=version+1,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND version=?",
