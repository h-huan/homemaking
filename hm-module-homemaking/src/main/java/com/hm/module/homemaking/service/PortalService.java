...
public class PortalService {
    ...
    private Map<String,Object> catalog(long tenant,Config config){
        ...
        Set<Long> reviews=requested.get("TESTIMONIALS");String ids=ids(reviews);var args=new ArrayList<Object>();args.add(tenant);String filter=ids.isEmpty()?"":" AND r.id IN ("+ids+")";var testimonials=repo.jdbc().queryForList("SELECT r.id,r.rating,r.service_rating,r.worker_rating,r.content,r.reply_content,r.tags_json,r.created_at,c.nickname FROM hm_review r JOIN hm_customer c ON c.id=r.customer_id WHERE r.tenant_id=? AND r.status='PUBLISHED' AND r.visible=TRUE"+filter+" ORDER BY r.recommended DESC,r.id DESC LIMIT 12",args.toArray());for(var review:testimonials)try{review.put("tags",json.readValue(Objects.toString(review.remove("tags_json"),"[]"),List.class));}catch(Exception ignored){review.put("tags",List.of());}out.put("reviews",testimonials);return out;
    }
