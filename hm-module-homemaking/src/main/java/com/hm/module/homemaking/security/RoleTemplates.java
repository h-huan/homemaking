...
public final class RoleTemplates {
    ...
    public static final Set<String> ALL = permissions("stores:read,write", "workers:read,write,bind", "services:read,write",
            ...
            "quota:read", "platform:manage", "worker:read,fulfill");
    private static Set<String> owner() {
        var p = new LinkedHashSet<>(ALL);
        p.remove("homemaking:platform:manage"); p.remove("homemaking:worker:read"); p.remove("homemaking:worker:fulfill");
    ...
    public static final List<Template> ALL_TEMPLATES = List.of(
        ...
        new Template("MANAGER", "店长", "STORES", permissions("stores:read,write", "workers:read,write,bind", "services:read,write", "orders:read,dispatch,reschedule,cancel,fulfill", "customers:read", "aftersales:read,reject", "reviews:read,moderate", "schedule:read,write")),
        ...
        new Template("FINANCE", "财务", "STORES", permissions("stores:read", "workers:read", "orders:read", "aftersales:read,refund", "finance:read,statement,approve,payout,reconcile")),
