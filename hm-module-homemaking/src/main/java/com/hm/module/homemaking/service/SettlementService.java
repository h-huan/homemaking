...
public class SettlementService {
    ...
    public Map<String,Object> rule(long tenant){AdminScope.tenant(tenant);var rows=repo.jdbc().queryForList("SELECT * FROM hm_commission_rule WHERE tenant_id=?",tenant);return rows.isEmpty()?Map.of("tenant_id",tenant,"platform_bps",0,"worker_bps",0,"cycle_days",30,"enabled",false,"version",0):rows.get(0);}
    @Transactional public void saveRule(long tenant,Rule r){
        AdminScope.platformOnly();check(r.platformBps()+r.workerBps()<=10000,"平台与人员分成合计不能超过 100%");
    ...
    @Transactional public void completed(Map<String,Object> order){
        ...
        entry(order,"PLATFORM",0,platformAmount,"complete:"+id);entry(order,"STORE",number(order,"store_id"),storeAmount,"complete:"+id);
    ...
    @Transactional public void refund(Map<String,Object> order,long aftersale,int amount){
        ...
        entry(order,"PLATFORM",0,-platform,"refund:"+aftersale);entry(order,"STORE",number(order,"store_id"),-store,"refund:"+aftersale);
    ...
    @Transactional public long statement(Statement s){
        AdminScope.tenant(s.tenantId());check(!s.periodEnd().isBefore(s.periodStart())&&s.periodEnd().isBefore(s.periodStart().plusDays(93)),"结算周期最多 93 天");
    ...
    public List<Map<String,Object>> statements(long tenant){AdminScope.tenant(tenant);return repo.jdbc().queryForList("SELECT * FROM hm_settlement_statement WHERE tenant_id=?"+repo.scope("hm_settlement_statement")+" ORDER BY id DESC LIMIT 200",tenant);}
    ...
    private Map<String,Object> lock(long id){var scope=AdminScope.current();boolean platform=scope!=null && scope.platform();var rows=platform?repo.jdbc().queryForList("SELECT * FROM hm_settlement_statement WHERE id=? FOR UPDATE",id):repo.jdbc().queryForList("SELECT * FROM hm_settlement_statement WHERE id=? AND tenant_id=?"+repo.scope("hm_settlement_statement")+" FOR UPDATE",id,repo.tenant());check(rows.size()==1,"结算单不存在");return rows.get(0);}
