...
public class HomemakingAppController {
    ...
    @GetMapping("/wechat/subscription-templates") public CommonResult<?> templates(@RequestParam String appId){long customer=access.current();
        ...
        return success(repo.jdbc().queryForList("SELECT DISTINCT template_id FROM hm_notification_template WHERE tenant_id=? AND channel='MINI' AND enabled=TRUE AND event_type IN ('SERVICE_REMINDER','SERVICE_COMPLETION_CONFIRM','WORKER_CHANGED','REFUND_RESULT','AFTERSALE_UPDATE') ORDER BY template_id LIMIT 3",String.class,repo.tenant()));}
