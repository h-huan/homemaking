...
public class EvidenceStorage {
    ...
    public Saved save(long tenant,byte[] bytes){
        check(bytes!=null&&bytes.length>0&&bytes.length<=5*1024*1024,"图片须为不超过 5 MB 的 JPEG 或 PNG");
        try(var input=ImageIO.createImageInputStream(new ByteArrayInputStream(bytes))){
            var readers=ImageIO.getImageReaders(input);check(readers.hasNext(),"无法识别图片格式");var reader=readers.next();
            try{
                String format=reader.getFormatName().toLowerCase();check(format.equals("jpeg")||format.equals("png"),"图片仅支持 JPEG 或 PNG");reader.setInput(input,true,true);
        ...
        }catch(IOException e){throw new IllegalStateException("图片保存失败，请检查私有存储目录",e);}
    }
    public byte[] read(String key){try{return Files.readAllBytes(resolve(key));}catch(IOException e){throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.NOT_FOUND,"图片不存在");}}
    public void deleteAfterCommit(Collection<String> keys){
        Runnable cleanup=()->keys.forEach(key->{try{Files.deleteIfExists(resolve(key));}catch(IOException ignored){}});
    ...
    private Path resolve(String key){Path path=root.resolve(key).normalize();check(path.startsWith(root)&&!path.equals(root),"图片路径无效");return path;}
}
